...
class PaymentTransaction(models.Model):
    ...
    def _process_feedback_data(self, data):
        ...
        if intent_status in con.INTENT_STATUS_MAPPING['draft']:
            pass
        elif intent_status in con.INTENT_STATUS_MAPPING['pending']:
            self._set_pending()
        elif intent_status in con.INTENT_STATUS_MAPPING['done']:
            if self.tokenize:
                self._stripe_tokenize_from_feedback_data(data)
            self._set_done()
        elif intent_status in con.INTENT_STATUS_MAPPING['cancel']:
            self._set_canceled()
